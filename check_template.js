...
/* 「調査中」は「相談」とほぼ同義（コワーク/チャットが案件作成時に両方の語で発動する）。
   営業ゾーンの同一段階として扱い、同じチェックリストを共有する。 */
CHECK_TEMPLATE['調査中'] = CHECK_TEMPLATE['相談'];

/* 「完了(請求書作成済み)」は旧称。実データは「完了」で登録されているため
   テンプレート側を「完了」に改名し、旧称はエイリアスで受ける（2026-08-25）。 */
CHECK_TEMPLATE['完了(請求書作成済み)'] = CHECK_TEMPLATE['完了'];

/* ------------------------------------------------------------
   終端・停止ステータスにテンプレートを定義しない理由（2026-08-25 判断）
   ------------------------------------------------------------
   「中止」「保留」「保守管理(継続)」はテンプレートを持たせない。

   - 終わった案件・止まった案件に進行チェックリストは要らない。
   - 定義しなくても壊れない。dashboard は「ワークフローを開く」ボタンを
     出さず、workflow.html を直接開くと『テンプレート未対応』と表示する。
   - description だけの空テンプレートを足しても、開くと
     『Phase 2 で実装予定』が出るだけで中身は無い。
   - getCarryOverItems は STATUS_ORDER.indexOf(status) <= 0 で [] を返すため、
     STATUS_ORDER に足さない限り持ち越し項目も出ない。
     つまり中身のあるチェックリストを出すには STATUS_ORDER への追加が要り、
     それはパイプライン順序としての意味を壊す。

   検討し直す場合は tools/kanban_check.py の [1-B] を見ること。
   ------------------------------------------------------------ */
